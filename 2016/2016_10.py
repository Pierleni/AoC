...
bots = collections.defaultdict(BOTS)
outs = collections.defaultdict(OUTS)
target = (61, 17) # 61, 17
...
while i < 30:
    for line in INPUT.split('\n'):
        ...
        if line[0] == 'value':
            ...
            bots[bot].get_value(value)

        elif line[0] == 'bot':
            bot, first, second = int(line[1]), int(line[6]), int(line[-1])

            low_el = bots if line[5] == 'bot' else outs
            high_el = bots if line[-2] == 'bot' else outs

            low_el[first].get_value(bots[bot].low)
            high_el[second].get_value(bots[bot].high)
    i += 1

for k, v in bots.items():
    if len(v.values) < 2: pass
    else:
        if v.values[0] in target and v.values[1] in target:
            print(f"\nResult part 1: bot n. {k} - {v.values}")
            print(f"Result part 2: {outs[0].values * outs[1].values * outs[2].values}")

# for k, v in bots.items():
#     print(f"BOT #{k} | low - high {v.low, v.high}")
#
# for k, v in outs.items():
#     print(f"OUT #{k} values = {v.values}")