...
int main()
{
    ...
    for (int i=0; i<=commands.size(); i++) {
        ...
        if (op == "L" || op == "R") {
            ...
            for (int j=0; j<step; j++) {
                ...
                if (!edgeFace(newPos)) continue;
                if (matrix[newPos.Y][newPos.X] == 2 ) continue;
                curr_pos = newPos;
                matrix[curr_pos.Y][curr_pos.X] = 9;
            }
        }
    ...
    cout << curr_pos.Y+1 << ' ' << curr_pos.X+1 << ' ' << facing[curr_dir] << '\n';
    std::cout << "Part 2: " << psw1 << endl; // right 138131
    return 0;
}
...
bool edgeFace(pt &p) {
    ...
    bool change = false;
    // L
    if (p.X == 49 && p.Y < 50) {tp.Y = 149 - p.Y; tp.X = 0; td = 'E'; change = true; }  // 1 < 4
    else if (p.X == 49 && (p.Y >= 50 && p.Y < 100)) {tp.Y = 100; tp.X = p.Y - 50; td = 'S'; change = true; } // 3<4
    else if (p.X < 0 && p.Y < 150) {tp.Y = 149 - p.Y; tp.X = 50; td = 'E'; change = true; } // 4 < 1
    else if (p.X < 0 && p.Y >= 150) {tp.Y = 0; tp.X = p.Y - 100; td = 'S'; change = true; } // 6 < 1

    // R
    else if (p.X == 150 && p.Y < 50) {tp.X = 99; tp.Y = 149 - p.Y; td = 'W'; change = true; } // 2 > 5
    else if (p.X == 100 && (p.Y >=50 && p.Y < 100)) {tp.X = p.Y + 50; tp.Y = 49; td = 'N'; change = true; } // 3 > 2
    else if (p.X == 100 && p.Y >=100) {tp.Y = 149 - p.Y; tp.X = 149; td = 'W'; change = true; } // 5 > 2
    else if (p.X == 50 && p.Y >=150) {tp.X = p.Y - 100; tp.Y = 149; td = 'N'; change = true; } // 6 > 5

    // U
    else if (p.Y == 99 && p.X < 50) {tp.Y = p.X + 50; tp.X = 50; td = 'E'; change = true; } // 4 ^ 3
    else if (p.Y < 0 && p.X < 100) {tp.Y = p.X + 100; tp.X = 0; td = 'E'; change = true; } // 1 ^ 6
    else if (p.Y < 0 && p.X >= 100) {tp.Y = 199; tp.X = p.X - 100; td = 'N'; change = true; } // 2 ^ 6

    // D
    else if (p.Y == 50 && p.X >= 100) {tp.Y = p.X - 50; tp.X = 99; td = 'W'; change = true; } // 2 v 3
    else if (p.Y == 150 && p.X >= 50) {tp.Y = p.X + 100; tp.X = 49; td = 'W'; change = true; } // 5 v 6
    else if (p.Y == 200 && p.X < 50) {tp.Y = 0; tp.X = p.X + 100; td = 'S'; change = true; } // 6 v 2

    
    if (!change) return true;
    else if (matrix[tp.Y][tp.X] == 2) return false;
    else {
        curr_dir = td;
        p = tp;
        return true;
