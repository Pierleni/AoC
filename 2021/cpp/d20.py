...
data = open("input/d20.txt", 'r').read().strip().split("\n\n")
alg = data.pop(0)
...
for i in range(1, 50 + 1):
    ...
    for r, c in changes:
        map[r][c] = changes[(r, c)]
    if i == 2:
        print(f"Part 1: {sum([val for sublist in map for val in sublist])}") #5489
print(f"Part 2: {sum([val for sublist in map for val in sublist])}") #19066