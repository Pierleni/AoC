...
while len(queue) > 0:
    for el in queue:
        a, b = el[0],el[1]      # IMPORTANTE non faccio queue.pop ma passo solamente tutti gli elemeti
        for dy, dx in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
            yy, xx = a + dy, b + dx
            if IsSafe(yy, xx):
                maze[yy][xx] = 5
                new_queue.add((yy,xx))  # creo un nuovo queue se IsSafe == True
    queue = new_queue                   # sostituisco il vecchio queue con il nuovo
    new_queue = set()                   # riazzero
    minutes += 1

os.system('cls')
for row in maze:
    print(''.join('\033[31m█' if x == 1 else '\033[97m█' if x == 5 else ' ' for x in row ))
