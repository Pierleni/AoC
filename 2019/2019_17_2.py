...
moves, steps = [], 0
while True:
    yy = directions[direct][0]
    xx = directions[direct][1]
    if matrix[y + yy][x + xx] == ord('#'):
        y, x = y + yy, x + xx
        steps += 1
        continue
    ...
    # if turn(left, y, x, direct): continue
    # elif turn(right, y, x, direct): continue
    # else: break

    new_direct = left[direct]                   # turn left
    yy = directions[new_direct][0]
    xx = directions[new_direct][1]
    if matrix[y + yy][x + xx] == ord('#'):
        direct = new_direct
        y, x = y + yy, x + xx
        steps = 0; moves.append('L')

    else:
        new_direct = right[direct]              # turn right
        yy = directions[new_direct][0]
        xx = directions[new_direct][1]
        if matrix[y + yy][x + xx] == ord('#'):
            direct = new_direct
            y, x = y + yy, x + xx
            steps = 0; moves.append('R')
